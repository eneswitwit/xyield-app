<div class="bg-blue-500 px-6 py-4 rounded-lg">
    <div class="flex">
        ...
        <div>
            <div class="text-sm font-medium text-white">Du bist in der Testphase des Kapitalanleger Abonnements.</div>
            <div class="text-xs text-gray-200">Deine Probezeit geht noch {{ auth()->user()->trial_ends_at->diffInDays(now()) }} Tage.</div>
    
        </div>
        @if(isset($action_btn))
            <a href="{{ route('billing') }}" class="text-white px-4 py-2 rounded border border-gray-300 self-end ml-auto">Upgrade durchführen</a>
        @endif
    </div>
</div>