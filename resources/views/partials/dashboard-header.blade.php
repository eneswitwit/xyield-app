            ...
            <div class="hidden lg:flex lg:items-center lg:w-auto w-full relative" id="menu">
                <nav>
                    <ul class="lg:flex items-center justify-between text-sm font-medium text-gray-700 pt-4 lg:pt-0">
                        <li><a class="lg:p-4 py-3 px-0 block border-b-2 border-transparent font-bold @if(Request::is('dashboard') || Request::is('home')){{ 'text-blue-500' }}@else{{ 'text-gray-600 hover:text-gray-900' }}@endif" href="{{ route('dashboard') }}">Applikation</a></li>
                        <li><a class="lg:p-4 py-3 px-0 block border-b-2 border-transparent font-bold @if(Request::is('support')){{ 'text-blue-500' }}@else{{ 'text-gray-600 hover:text-gray-900' }}@endif lg:mb-0 mb-2" href="{{ route('support') }}">Support</a></li>
                        <li><a class="lg:p-4 py-3 px-0 block border-b-2 border-transparent @if(Request::is('announcements/unread')){{ 'text-blue-500 font-bold' }}@else{{ 'text-gray-500 hover:text-gray-700' }}@endif lg:mb-0 mb-2 relative" href="{{ route('announcements.unread') }}">
                            @if( auth()->user()->hasUnreadAnnouncements() )
                                <span class="bg-blue-500 w-2 h-2 rounded-full absolute left-0 top-0 ml-3 mt-4 announcement-indicator"></span>
                            @endif
                ...
                <a href="#" class="group lg:ml-4 flex items-center justify-start lg:mb-0 mb-4 pointer-cursor border-l border-gray-300 pl-6" id="userdropdown">
                    <p class="font-bold text-xs pr-2 text-gray-700 text-right ignore-body-click">{{ auth()->user()->name }}<br>
                        @if(auth()->user()->onTrial())
                            <span class="text-xs text-blue-500 ignore-body-click">{{ ucfirst(auth()->user()->plan->title) }} Testphase</span>
                        @else
                            <span class="text-xs text-blue-600 ignore-body-click">{{ ucfirst(auth()->user()->plan->title) }} </span>
                        @endif
                    </p>
                    <img class="rounded-full w-10 h-10 border-2 border-gray-300 group-hover:border-blue-500 ignore-body-click" src="{{ auth()->user()->photo }}" alt="avatar">
                </a>
                <div id="usermenu" class="absolute lg:mt-12 pt-1 z-40 left-0 lg:left-auto lg:right-0 lg:top-0 invisible lg:w-auto w-full">
                    <div class="bg-white shadow-xl lg:px-8 px-6 lg:py-4 pb-4 pt-0 rounded rounded-t-none">
                        <a href="/settings" class="pb-2 block text-gray-600 hover:text-gray-900 font-medium ignore-body-click">Einstellungen</a>
                        <a href="/logout" class="block text-gray-600 hover:text-gray-900 font-medium ignore-body-click">Abmelden</a>
                    </div>
                </div>
